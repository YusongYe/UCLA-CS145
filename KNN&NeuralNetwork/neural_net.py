...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        def softmax_loss(x, y):
            ...
            probs = expo / np.sum(expo, axis=1, keepdims=True)
        ...
        def MSE_loss(y_pred, y_target):
            ...
            for i in range(n):
                for j in range(len(self.params['b2'])):
                    if j == y_target[i]:
                        y_temp[i][j] = 1
            
            sum_y = np.sum((y_pred - y_temp)**2)
            loss = sum_y/(2*n)
            dy_pred= (y_pred - y_temp)/n
        ...
        # ================================================================ #
        reg_loss = 0.5 * reg * (np.sum(np.power(W1,2)) + np.sum(np.power(W2,2)))
