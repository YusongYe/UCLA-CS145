...
def getBeta(train_x, train_y):   
    ...
    inverse_x = np.linalg.inv(np.matmul(trans_x,train_x))
    beta = np.matmul(np.matmul(inverse_x , trans_x) , train_y)
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            ...
            temp = (y- np.matmul(np.transpose(x),beta)) * lr
            delta = x.dot(temp)
