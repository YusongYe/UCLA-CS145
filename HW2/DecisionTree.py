...
def compute_info_gain(data, att_name, y_name):
    ...
    for val, count in zip(vals, counts):
        att_data = data[data[att_name] == val]
        conditional_entropy += count / total_counts * compute_entropy(att_data[y_name])
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    ########## Please Fill Missing Lines Here ##########
    for count in counts:
        att_info -= count / total_counts * np.log2(count / total_counts)
